...
Scored against `data/eval_sample.csv` — 180 reviews, disjoint from the 180-review sample used to build the taxonomy, never seen by the tagging prompt as few-shot material. Predictions pulled from `tagged_reviews_eval_variantB_fewshot_only.csv` (variant B: fewshot-only, eval-only re-tag).

## Headline metrics

- **Subcategory-level micro-F1**: 0.620 (precision 0.579, recall 0.668)
- **Subcategory-level macro-F1**: 0.597 (all 39 subcategories, zero-support ones counted) / 0.631 (only the 35 subcategories with at least 1 human-labeled example in this eval set)
- **Parent-level micro-F1**: 0.762 (precision 0.728, recall 0.799)
- **Parent-level macro-F1**: 0.711 / 0.770 (only the 12 parents with support in this eval set)
- **Multi-label exact-match rate** (subcategory set == human set exactly): 38.9%
- **Multi-label exact-match rate** (parent set): 53.9%
- **Mean per-review Jaccard similarity** (subcategory level, partial credit): 0.570
- **Other/Ungrouped agreement**: of 24 reviews the human left blank, the tagger also predicted zero tags on 19 (79.2%) and forced at least one tag onto 5 (20.8%)
...
|---|---|---|---|---|---|---|---|
| metric_accuracy | 36 | 21 | 1 | 15 | 95.5% | 58.3% | 0.724 |
| feature_ui_friction | 32 | 8 | 5 | 24 | 61.5% | 25.0% | 0.356 |
...
| data_sync_delays | 7 | 6 | 4 | 1 | 60.0% | 85.7% | 0.706 |
...
| missing_language_support | 2 | 2 | 0 | 0 | 100.0% | 100.0% | 1.000 |
| redesign_regression | 2 | 2 | 20 | 0 | 9.1% | 100.0% | 0.167 |
...
| sleep_alarm_ui | 1 | 1 | 2 | 0 | 33.3% | 100.0% | 0.500 |
...
|---|---|---|---|---|---|---|---|
| data_accuracy | 44 | 24 | 1 | 20 | 96.0% | 54.5% | 0.696 |
| ui_ux | 34 | 23 | 9 | 11 | 71.9% | 67.6% | 0.697 |
| sync_connectivity | 33 | 30 | 4 | 3 | 88.2% | 90.9% | 0.896 |
| pricing_billing | 32 | 32 | 8 | 0 | 80.0% | 100.0% | 0.889 |
| app_stability | 20 | 14 | 9 | 6 | 60.9% | 70.0% | 0.651 |
| sleep_tracking | 20 | 17 | 6 | 3 | 73.9% | 85.0% | 0.791 |
...
| localization | 2 | 2 | 0 | 0 | 100.0% | 100.0% | 1.000 |
...
| data_accuracy vs health_signal_reliability | 1 | 0 | 1 | 20 | 14 |
...
- **[MISMATCH]** review_id `14177657644` [app_store, 5-star]
  ...
  - human: ai_positive_reception;metric_accuracy
  - predicted: ai_positive_reception;ecg_bp_feature_reliability;metric_accuracy;price_value_perception
...
- **[MISMATCH]** review_id `13766483189` [app_store, 4-star]
  ...
  - predicted: ecg_bp_feature_reliability
...
### review_id `13766483189` [app_store, 4-star] — Jaccard 0.00

> You can’t swipe up to blood pressure insights on the health tab in the last two versions it just hangs. Fix it please.

- **Human**: crashes_freezes;feature_ui_friction
  - human notes: Reviewer cannot access the blood pressure insights screen because the swipe interaction hangs in the last two app versions. This is an interaction and stability issue, not a complaint about the accuracy or reliability of the blood pressure feature itself.
- **Predicted**: ecg_bp_feature_reliability
